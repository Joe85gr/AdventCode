...
namespace Day8
{
    public static class SecondPart
    {
        ...
        public static double GetResult(IEnumerable<string> fileLines)
        {
            ...
            foreach (var line in patterns)
            {
                ...
                foreach (var number in pattern)
                {
                    ...
                    if (uniqueNumberSegments.ContainsKey(digits) == false) continue;
        ...
        private static void FindNumberPattern(Dictionary<string, bool> patternLeft, IDictionary<int, string> numberMaps, int criteria1, int criteria2, int number)
        {
            var pattern = string.Empty;

            foreach (var (key, _) in patternLeft)
            {
                var chrLeft = RemoveChars(key, numberMaps[criteria1]).Length;
